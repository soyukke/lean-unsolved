...
/-- 奇数 n に対して (3n+1)/2 の mod 3 は n の mod 3 で決まる:
    具体的には ((3n+1)/2) % 3 = (n+2) % 3 * (n%2) + (1-n%2) * (n/2 % 3) の代わりに、
    直接的に: 奇数 n に対して (3n+1)/2 % 3 と (n%3) の関係。
    n%3=0 → (3n+1)/2 % 3: 3|n のとき 3n+1 ≡ 1 (mod 3) で (3n+1)/2 の mod 3 は n%6 依存。
